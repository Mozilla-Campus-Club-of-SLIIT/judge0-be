...
func GetLiveLeaderboard(ctx context.Context, page, pageSize string) ([]types.LeaderboardUserType, int64, int64, error) {
	pool := database.GetPool()
	ctx, cancel := utils.WithTimeout(ctx)
	defer cancel()
	...
	err := pool.QueryRow(ctx,
		"SELECT count(*) FROM leaderboard").Scan(&count)
	if err != nil {
		logger.Log.Error("GetLiveLeaderboard: error counting leaderboard entries", "error", err)
		return nil, 0, 0, err
	}

	ps, err := strconv.ParseInt(pageSize, 10, 64)
	if err != nil || ps <= 0 {
		logger.Log.Warn("GetLiveLeaderboard: invalid pageSize, defaulting to 10", "input", pageSize, "error", err)
		ps = 10
	}

	p, err := strconv.ParseInt(page, 10, 64)
	if err != nil || p <= 0 {
		logger.Log.Warn("GetLiveLeaderboard: invalid page, defaulting to 1", "input", page, "error", err)
		p = 1
	}
	...
	rows, err := pool.Query(ctx,
	 ...
	 LIMIT $1 OFFSET $2`,
		ps, offset)
	if err != nil {
		logger.Log.Error("GetLiveLeaderboard: query error", "error", err)
		return nil, 0, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			logger.Log.Error("GetLiveLeaderboard: scan error", "error", err)
			return nil, 0, 0, err
		}
		users = append(users, user)
	}
	...
	if err := rows.Err(); err != nil {
		logger.Log.Error("GetLiveLeaderboard: rows error", "error", err)
		return nil, 0, 0, err
	}

	logger.Log.Info("Fetched live leaderboard", "count", len(users), "page", p, "totalPages", totalPages)
	return users, p, totalPages, nil
}
